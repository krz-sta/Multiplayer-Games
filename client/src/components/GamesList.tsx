...
function GamesList({ socket, currentRoom, setCurrentRoom, selectedGame, setSelectedGame }: GamesListProps) {
    ...
    if (selectedGame === 'tictactoe') {
        return (
            <div className="flex flex-col md:flex-row flex-1 h-full overflow-hidden">
                ...
                {/* Mobile top bar with back + lobby toggle */}
                <div className="flex md:hidden items-center justify-between px-4 py-3 bg-slate-800/50 border-b border-slate-700/50">
                    <button
                        onClick={() => setSelectedGame(null)}
                        className="bg-slate-700/50 border border-slate-600/50 text-slate-300 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-slate-700 transition-all active:scale-95"
                    >
                        ← Back
                    </button>
                    <button
                        onClick={() => setSidebarOpen(!sidebarOpen)}
                        className="bg-slate-700/50 border border-slate-600/50 text-slate-300 px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-slate-700 transition-all active:scale-95 flex items-center gap-1.5"
                    >
                        <span>👥</span>
                        <span>Lobby</span>
                        <span className="bg-indigo-500/30 text-indigo-300 text-xs px-1.5 py-0.5 rounded-full ml-1">{playersInRoom.length}</span>
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 transition-transform ${sidebarOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {sidebarOpen && (
                    <div className="md:hidden bg-slate-800/50 border-b border-slate-700/50 p-4 overflow-y-auto max-h-[60vh]">
                        <h2 className="font-bold text-sm text-slate-400 uppercase tracking-wider mb-2">Players in lobby</h2>
                        <ul className="mb-4 bg-slate-900/30 rounded-xl p-2 border border-slate-700/30">
                            {playersInRoom.map(p => (
                        ...
                        {friends.length === 0 ? (
                            <p className="text-center text-slate-500 text-sm py-2">No friends added yet.</p>
                        ) : (
                ...
                <div className="hidden md:flex w-80 border-r border-slate-700/50 p-4 flex-col overflow-y-auto bg-slate-800/30">
                    <button
                        onClick={() => setSelectedGame(null)}
                ...
                <div className="flex-1 p-4 md:p-6 flex flex-col items-center overflow-y-auto">
                    ...
                    {!gameActive && !winner && (
                        <div className="flex flex-col items-center flex-none mb-6 md:mb-8">
                            <h2 className="text-xl md:text-2xl font-bold text-white mb-3 md:mb-4">Ready to play?</h2>
                            <button
                                onClick={handleReady}
                                disabled={isReady}
                                className={`px-6 py-3 md:px-8 md:py-4 rounded-xl font-bold text-white shadow-lg transition-all text-lg md:text-xl active:scale-95 ${isReady
                    ...
                    {(gameActive || winner) && (
                        <div className="flex flex-col items-center flex-none mb-6 md:mb-8">
                            ...
                            {mySymbol && !winner && (
                                <p className="text-sm text-slate-400 mb-3 md:mb-4">
                                    Playing as: <span className={`font-bold ${mySymbol === 'X' ? 'text-indigo-400' : 'text-rose-400'}`}>{mySymbol}</span>
                                </p>
                            )}

                            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 p-2 sm:p-3 rounded-2xl shadow-2xl">
                                {board.map((cell, index) => (
                                    <div
                                        key={index}
                                        onClick={() => handleCellClick(index)}
                                        className={`w-[4.5rem] h-[4.5rem] sm:w-20 sm:h-20 md:w-24 md:h-24 bg-slate-900/50 rounded-xl flex items-center justify-center text-3xl sm:text-4xl md:text-5xl font-black shadow-inner transition-all duration-150
                            ...
                            {winner && (
                                <button
                                    onClick={handleReady}
                                    disabled={isReady}
                                    className={`mt-4 md:mt-6 px-5 py-2.5 md:px-6 md:py-3 rounded-xl font-bold text-white shadow-lg transition-all active:scale-95 ${isReady
                                        ? 'bg-slate-600 cursor-not-allowed'
                    ...
                    <div className="flex flex-col flex-1 w-full max-w-2xl mt-auto">
                        ...
                        <div className="flex-1 bg-slate-800/30 rounded-2xl border border-slate-700/30 mb-3 p-3 md:p-4 overflow-y-auto min-h-[120px] max-h-[200px] md:min-h-[150px] md:max-h-[250px]">
                        ...
                        <div className="flex gap-2">
                            <input
                                type="text"
                                className="flex-1 px-3 py-2.5 md:px-4 md:py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                            ...
                            <button
                                onClick={sendMessage}
                                className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-bold px-4 py-2.5 md:px-6 md:py-3 rounded-xl hover:from-indigo-600 hover:to-purple-600 shadow-lg shadow-indigo-500/20 transition-all active:scale-95 text-sm md:text-base"
                            >
                                Send
                            </button>
                        </div>
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 flex-1">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">Available games</h2>
            <p className="text-slate-400 mb-4 sm:mb-6 text-sm sm:text-base">Choose a game to play with friends</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                {gameRegistry.map(game => (
                    <div
                        key={game.id}
                        onClick={() => { resetGame(); setSelectedGame(game.id); }}
                        className="aspect-square bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500/50 hover:shadow-xl hover:shadow-indigo-500/10 hover:scale-105 transition-all duration-200 gap-2 sm:gap-3 p-3 sm:p-4 group"
                    >
                        <span className="text-3xl sm:text-4xl group-hover:scale-110 transition-transform">{game.icon}</span>
                        <span className="font-bold text-base sm:text-lg text-white text-center">{game.name}</span>
                        <span className="text-xs text-slate-400 text-center leading-tight hidden sm:block">{game.description}</span>
                    </div>
                ))}
            </div>
        </div>
